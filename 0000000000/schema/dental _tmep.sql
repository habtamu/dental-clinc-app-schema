...
drop type if exists dental.letterresult CASCADE;
...
CREATE TYPE dental.letterresult AS (
...
select t."startat" as "date", t.endat,t.patnumber,t.seqno, t.visittype,t.registerby,c.name as cc, t.printcount
...
END;
$$ LANGUAGE PLPGSQL;
create or replace function dental.get_lettersbypatient(INTEGER)
returns setof dental.letterresult
as $$
DECLARE
  patnum alias for $1; 
  outrow dental.letterresult;
BEGIN
SET join_collapse_limit = 1;
	FOR outrow IN
with CTE_letter AS (
 select patnumber,seqno,examdate,refno,registerby,lastmodifyby,(print).printcount 
 from dental.letters
)
select t.examdate, t.patnumber,t.seqno, t.refno,t.registerby,t.lastmodifyby, t.printcount
from CTE_letter as t
where t.patnumber = patnum
